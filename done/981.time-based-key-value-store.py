...
class TimeMap:
    ...
    def get(self, key: str, timestamp: int) -> str:
        ...
        timestamped_values = self.map[key]

        if timestamp >= timestamped_values[-1][0]:
            return timestamped_values[-1][1]
        elif timestamp < timestamped_values[0][0]:
            return ""
        
        found = bisect.bisect_left(timestamped_values, timestamp, key=lambda x:x[0])
        found -= 1 if timestamped_values[found][0] != timestamp else 0

        return timestamped_values[found][1]
