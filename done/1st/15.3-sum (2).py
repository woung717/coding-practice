    ...
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        for i in range(len(nums) - 2):
            for j in range(i + 1, len(nums) - 1):
                ...
                if remain in index and i != index[remain] and j != index[remain]:
                    if remain > nums[j]:
                        ret.add((nums[i], nums[j], remain))
                    elif remain < nums[i]:
                        ret.add((remain, nums[i], nums[j]))
                    else:
                        ret.add((nums[i], remain, nums[j]))
        
        return [list(n) for n in ret]
