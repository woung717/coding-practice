...
class Solution:
    def accountsMerge(self, accounts):
        class UnionFind:
            ...
            def find(self, x):
                y = self.parent_dict.get(x, x)
                if y != x:
                    self.parent_dict[x] = y = self.find(y)
                return y
            
            def union(self, x, y):
                self.parent_dict[self.find(x)] = self.find(y)
        ...
        for i, account in enumerate(accounts):
            for email in account[1:]:
                if email not in account_dict:
                    account_dict[email] = i
                else:
                    union_find.union(account_dict[email], i)

        email_groups = defaultdict(list)
        for account, i in account_dict.items():
            rep_i = union_find.find(i)
            email_groups[rep_i].append(account)
        
        return [[accounts[i][0]] + sorted(emails) for i, emails in email_groups.items()]
