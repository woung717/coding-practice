...
class Solution:
    def accountsMerge(self, accounts):
        ...
        class UnionFind:
            ...
            def find(self, x):
                found = self.rep_dict.get(x, x)
                if found != x:
                    self.rep_dict[x] = found = self.find(found)

                return found
            
            def union(self, x, y):
                self.rep_dict[self.find(x)] = self.find(y)
        ...
        for i, account in enumerate(accounts):
            for email in account[1:]:
                if email not in email_dict:
                    ...
                    union_find.union(i, email_dict[email])
        
        emails = defaultdict(list)
        for email, i in email_dict.items():
            rep = union_find.find(i)
            emails[rep].append(email)
        
        return [[accounts[i][0]] + sorted(email_group) for i, email_group in emails.items()]
